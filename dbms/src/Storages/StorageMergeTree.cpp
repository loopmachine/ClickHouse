...
void StorageMergeTree::clearColumnInPartition(const ASTPtr & partition, const Field & column_name, const Settings &)
{
...
bool StorageMergeTree::optimize(
    const ASTPtr & query, const ASTPtr & partition, bool final, bool deduplicate, const Settings & settings)
{
    String partition_id;
    if (partition)
...
void StorageMergeTree::dropPartition(const ASTPtr & query, const ASTPtr & partition, bool detach, const Settings & settings)
{
    /// Asks to complete merges and does not allow them to start.
    /// This protects against "revival" of data for a removed partition after completion of merge.
    auto merge_blocker = merger.cancel();
...
void StorageMergeTree::attachPartition(const ASTPtr & partition, bool part, const Settings & settings)
{
    String partition_id;
...
void StorageMergeTree::freezePartition(const ASTPtr & partition, const String & with_name, const Settings & settings)
{
