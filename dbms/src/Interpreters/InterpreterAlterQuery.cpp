    ...
    for (const PartitionCommand & command : partition_commands)
    {
        switch (command.type)
        {
            ...
            case PartitionCommand::ATTACH_PARTITION:
                table->attachPartition(command.partition, command.part, context.getSettingsRef());
                break;
            ...
            case PartitionCommand::CLEAR_COLUMN:
                table->clearColumnInPartition(command.partition, command.column_name, context.getSettingsRef());
                break;
        }
    }
